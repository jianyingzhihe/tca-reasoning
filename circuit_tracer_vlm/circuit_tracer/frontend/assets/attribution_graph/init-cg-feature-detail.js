window.initCgFeatureDetail = async function({visState, renderAll, data, cgSel}){
  ...
  var renderFeatureExamples = util.throttleDebounce(featureExamples.renderFeature, 2000)

  var minCtxIdx = Infinity;
  for (const d of data.nodes) {
      if (d.feature_type === 'embedding' && d.clerp && d.clerp.includes('image_soft_token')) {
          if (d.ctx_idx !== undefined && d.ctx_idx < minCtxIdx) {
              minCtxIdx = d.ctx_idx;
          }
      }
  }
  if (minCtxIdx === Infinity) {
      minCtxIdx = null;
  }
  ...
  function renderFeatureDetail() {
    ...
    if (d.isError || d.feature_type == 'embedding' || d.feature_type == 'logit'){
      ...
      var headerDiv = headerSel.html('').append('div')
      headerDiv.append('div.header-top-row').append('div.feature-title')
        .text(d.ppClerp)
      if (d.clerp.includes('image_soft_token')) {
        headerDiv.append('div')
          .html(`<img src="./emb_image/${d.ctx_idx-minCtxIdx}" alt="image" style="margin-top: 10px;">`)
      }
      examplesSel.st({opacity: 0})
