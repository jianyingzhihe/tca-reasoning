...
def _target_stats(logits: torch.Tensor, target_token_id: int, tokenizer) -> dict[str, str]:
    row = logits[0, logits.shape[1] - 1].detach().float().cpu()
    ...
    competitor_id = int(sorted_ids[0].item()) if int(sorted_ids[0].item()) != target_token_id else int(sorted_ids[1].item())
    ...
    return {
        ...
        "target_vs_competitor_margin": f"{target_logit - competitor_logit:.10g}",
        "top5_token_ids": ",".join(str(x) for x in top5),
        "top5_tokens": " || ".join(_decode_token(tokenizer, x) for x in top5),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate target rank/margin under evidence-region masks without node intervention.")
    ...
    with torch.no_grad():
        for row in manifest_rows:
            ...
            for condition in conditions:
                ...
                stats = _target_stats(logits, int(row["target_token_id"]), processor.tokenizer)
                results.append(
                    {
                ...
                print(
                    ...
                    f"condition={condition} rank={stats['target_rank']} margin={stats['target_vs_competitor_margin']}",
                    flush=True,
                )
