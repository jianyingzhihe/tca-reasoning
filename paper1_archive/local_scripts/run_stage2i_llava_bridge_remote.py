...
.venv/bin/python -u scripts/research/run_cross_model_hidden_position_decode_smoke.py \\
  ...
  --work-dir "$STAGE/stage2i_llava_bridge_work" \\
  --sample-manifest "$STAGE/stage2i_selected_12_manifest.csv" \\
  --samples manifest \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  --groups top_hidden_delta_plus_answer_adjacent,evidence_region_plus_answer_adjacent,answer_adjacent_text,low_delta_control,random_control_1 \\
  --directions restore \\
  --max-new-tokens 3 \\
  ...
  --random-controls 4 \\
  --out-json "$STAGE/stage2i_llava_decoded_bridge_expansion.json" \\
  --out-csv "$STAGE/stage2i_llava_decoded_bridge_expansion.csv"
"""
...
def _run_local_analysis() -> None:
    inputs = [
        ...
        LOCAL_STAGE / "stage2i_llava_decoded_bridge_expansion.csv",
    ]
    existing = [str(path) for path in inputs if path.exists()]
    if not existing:
        print("skip analysis: no Stage 2I bridge CSV outputs")
        return
    cmd = [
        sys.executable,
        ...
        ",".join(existing),
        "--out-summary",
        str(LOCAL_STAGE / "stage2i_cross_model_bridge_summary.csv"),
        "--out-case-table",
        str(LOCAL_STAGE / "stage2i_cross_model_bridge_case_table.csv"),
        "--out-decision",
        str(LOCAL_STAGE / "stage2i_cross_model_bridge_decision.json"),
    ]
...
def main() -> int:
    ...
    remote_script = f"{REMOTE_STAGE}/run_stage2i_llava_bridge.sh"
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 21600:
            ...
            raise TimeoutError("Stage 2I LLaVA bridge exceeded 6 hours")
    ...
    sftp = client.open_sftp()
    for name in [
        "stage2i_llava_decoded_bridge_expansion.json",
        "stage2i_llava_decoded_bridge_expansion.csv",
    ]:
        try:
