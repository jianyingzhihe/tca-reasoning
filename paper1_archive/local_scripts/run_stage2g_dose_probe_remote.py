...
def main() -> int:
    ...
    remote_script = f"{REMOTE_STAGE}/run_stage2g_dose_probe.sh"
...
.venv/bin/python -u scripts/research/run_cross_model_feature_intervention_smoke.py \\
  ...
  --work-dir "$STAGE/stage2g_qwen_dose_work" \\
  ...
  --scales 1,3,5 \\
  --signed-probe \\
  --out-json "$STAGE/stage2g_qwen_feature_intervention_dose_probe.json" \\
  --out-csv "$STAGE/stage2g_qwen_feature_intervention_dose_probe.csv"

echo '--- LLaVA dose/sign probe ---'
.venv/bin/python -u scripts/research/run_cross_model_feature_intervention_smoke.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --transcoder-ref KokosDev/llava15-7b-clt \\
  --annotation-roots "$ASSET_ROOT,/root/autodl-tmp/tca-reasoning/annotation/okvqa_evidence_labelme_round4_core24_easy,/root/autodl-tmp/tca-reasoning/annotation/okvqa_evidence_labelme_round4_mainline16" \\
  --work-dir "$STAGE/stage2g_llava_dose_work" \\
  --samples okvqa_val_2847255,okvqa_val_4157235,okvqa_val_3658865 \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  --top-k-features 4 \\
  --scales 1,3,5 \\
  --signed-probe \\
  --out-json "$STAGE/stage2g_llava_feature_intervention_dose_probe.json" \\
  --out-csv "$STAGE/stage2g_llava_feature_intervention_dose_probe.csv"
"""
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 10800:
            ...
            raise TimeoutError("Stage 2G dose probe exceeded 3 hours")
    ...
    sftp = client.open_sftp()
    for name in [
        "stage2g_qwen_feature_intervention_dose_probe.json",
        "stage2g_qwen_feature_intervention_dose_probe.csv",
        "stage2g_llava_feature_intervention_dose_probe.json",
        "stage2g_llava_feature_intervention_dose_probe.csv",
    ]:
        try:
