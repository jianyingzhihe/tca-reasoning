...
REMOTE_STAGE = "/root/autodl-tmp/tca-reasoning/stage4_qwen_evidence_specific_nodes"
REMOTE_ASSETS = f"{REMOTE_STAGE}/assets"
LOCAL_CROSS = ROOT / "doc" / "experiments" / "stage4" / "cross_model"
MANIFESTS = {
    "primary": LOCAL_CROSS / "stage4_qwen_evidence_specific_nodes_primary_manifest.csv",
    "strict": LOCAL_CROSS / "stage4_qwen_evidence_specific_nodes_strict_manifest.csv",
}
...
def _parse_layers(raw: str, manifest: Path) -> list[int]:
    if raw in {"selected", "all"}:
        layers = sorted({int(float(row["layer"])) for row in _read_csv(manifest) if row.get("layer")})
        return layers
    if raw == "main":
        return [11, 13, 15, 17]
    if raw == "control":
...
def _upload_assets(base, sftp, manifest: Path, pack: str) -> None:
    ...
    base._put_file(sftp, manifest, f"{REMOTE_STAGE}/{pack}_manifest.csv")
    ...
    for row in rows:
        ...
        if image.exists() and image_name:
            ...
            if remote_image in seen_remotes:
                continue
        ...
        for key, out_name in [
            ...
            if mask.exists() and stem:
                ...
                if _put_if_missing(base, sftp, mask, remote_mask):
                    ...
                    skipped += 1
    print(
        f"uploaded evidence-specific assets: rows={len(rows)} unique_remote_paths={len(seen_remotes)} uploaded={uploaded} skipped={skipped}",
        flush=True,
    )


def _remote_script(pack: str, mode: str, layers: list[int], tag: str, smoke_candidates: int, scale: float, selection: str) -> str:
    layers_py = ",".join(str(layer) for layer in layers)
    prefix_base = f"{PREFIX}_{pack}_{mode}"
    suffix = f"_{tag}" if tag else ""
    filtered_manifest = f"{prefix_base}{suffix}_manifest.csv"
    layer_blocks = []
    for layer in layers:
        ...
        layer_blocks.append(
            f"""
echo '--- Stage4-052 evidence-specific {pack} {mode} L{layer} ---'
.venv/bin/python - <<PY
import csv
from pathlib import Path
...
echo '--- Stage4-052 Qwen evidence-specific nodes disk/gpu ---'
...
import csv
from pathlib import Path
src = Path("$STAGE/{pack}_manifest.csv")
...
if selection == "main":
    rows = [row for row in rows if row.get("include_main") == "1"]
elif selection == "sensitivity":
    rows = [row for row in rows if row.get("include_sensitivity_rank10") == "1"]
else:
...
date '+%Y-%m-%d %H:%M:%S %Z %z'
echo PROCS
ps -eo pid,ppid,stat,etime,pcpu,pmem,args | grep -E 'stage4_qwen_evidence_specific_nodes|run_stage4_qwen_causal_cutter_validation.py|{patterns}' | grep -v grep || true
echo FILES
ls -lh {REMOTE_STAGE}/{PREFIX}_{pack}_{mode}_* 2>/dev/null || true
echo LOGS
ls -lh {REMOTE_STAGE}/logs/*evidence_specific* 2>/dev/null || true
echo GPU
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Stage4-052 Qwen evidence-specific node validation.")
    ...
    parser.add_argument("--selection", choices=["main", "all", "sensitivity"], default="main")
    ...
    if args.selection != "main" and not args.tag:
        args.tag = args.selection

    manifest = MANIFESTS[args.pack]
    if not manifest.exists() and not args.status:
        subprocess.run([sys.executable, str(ROOT / "scripts" / "local" / "build_stage4_qwen_evidence_specific_node_manifest.py")], check=True)
    ...
    if args.fetch_only:
        ...
        if not args.skip_analyze:
            analyze_cmd = [sys.executable, str(ROOT / "scripts" / "local" / "analyze_stage4_qwen_evidence_specific_node_validation.py")]
            if args.tag:
    ...
    _upload_assets(base, sftp, manifest, args.pack)

    remote_script = f"{REMOTE_STAGE}/run_stage4_qwen_evidence_specific_nodes_{args.pack}_{args.mode}{('_' + args.tag) if args.tag else ''}.sh"
    with sftp.file(remote_script, "w") as handle:
        handle.write(_remote_script(args.pack, args.mode, layers, args.tag, args.smoke_candidates, args.scale, args.selection).replace("\r\n", "\n"))
    ...
    if args.detach:
        ...
        remote_log = f"{remote_log_dir}/evidence_specific_{args.pack}_{args.mode}_{stamp}.log"
