...
def main() -> int:
    ...
    remote_script = f"{REMOTE_STAGE}/run_stage2h_decoded_answer_smoke.sh"
...
.venv/bin/python -u scripts/research/run_cross_model_hidden_position_decode_smoke.py \\
  ...
  --work-dir "$STAGE/stage2h_qwen_decode_work" \\
  ...
  --random-controls 4 \\
  --out-json "$STAGE/stage2h_qwen_decoded_answer_smoke.json" \\
  --out-csv "$STAGE/stage2h_qwen_decoded_answer_smoke.csv"

echo '--- LLaVA decoded answer restore smoke ---'
.venv/bin/python -u scripts/research/run_cross_model_hidden_position_decode_smoke.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --annotation-roots "$ASSET_ROOT,/root/autodl-tmp/tca-reasoning/annotation/okvqa_evidence_labelme_round4_core24_easy,/root/autodl-tmp/tca-reasoning/annotation/okvqa_evidence_labelme_round4_mainline16" \\
  --work-dir "$STAGE/stage2h_llava_decode_work" \\
  --samples okvqa_val_2847255,okvqa_val_4157235,okvqa_val_3658865 \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  ...
  --answer-adjacent-count 4 \\
  --random-controls 4 \\
  --out-json "$STAGE/stage2h_llava_decoded_answer_smoke.json" \\
  --out-csv "$STAGE/stage2h_llava_decoded_answer_smoke.csv"
"""
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 10800:
            ...
            raise TimeoutError("Stage 2H decoded answer smoke exceeded 3 hours")
    ...
    for name in [
        "stage2h_qwen_decoded_answer_smoke.json",
        "stage2h_qwen_decoded_answer_smoke.csv",
        "stage2h_llava_decoded_answer_smoke.json",
        "stage2h_llava_decoded_answer_smoke.csv",
    ]:
        try:
