...
def _upload_assets(base, sftp, rows: list[dict[str, str]]) -> None:
    ...
    print(f"uploaded/verified Stage 2L assets for {len(rows)} selected samples")
...
echo '--- Stage 2L mask-shuffled negative control disk/gpu ---'
...
echo '--- Stage 2L Qwen mask-shuffled negative control ---'
.venv/bin/python -u scripts/research/run_cross_model_mask_shuffled_negative_control_smoke.py \\
  ...
  --work-dir "$STAGE/stage2l_qwen_mask_shuffled_work" \\
  ...
  --random-controls 1 \\
  --out-json "$STAGE/stage2l_qwen_mask_shuffled_negative_control.json" \\
  --out-csv "$STAGE/stage2l_qwen_mask_shuffled_negative_control.csv"

echo '--- Stage 2L LLaVA mask-shuffled negative control ---'
.venv/bin/python -u scripts/research/run_cross_model_mask_shuffled_negative_control_smoke.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --annotation-roots "$ASSET_ROOT" \\
  --work-dir "$STAGE/stage2l_llava_mask_shuffled_work" \\
  --sample-manifest "$STAGE/stage2i_selected_12_manifest.csv" \\
  --samples manifest \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  ...
  --answer-adjacent-count 4 \\
  --random-controls 1 \\
  --out-json "$STAGE/stage2l_llava_mask_shuffled_negative_control.json" \\
  --out-csv "$STAGE/stage2l_llava_mask_shuffled_negative_control.csv"
"""
...
def _run_local_analysis() -> None:
    cmd = [
        sys.executable,
        str(ROOT / "vlm-circuit-tracing" / "circuit_tracer_vlm" / "scripts" / "research" / "analyze_stage2l_mask_shuffled_control.py"),
    ]
...
def main() -> int:
    ...
    remote_script = f"{REMOTE_STAGE}/run_stage2l_mask_shuffled_negative_control.sh"
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 21600:
            ...
            raise TimeoutError("Stage 2L mask-shuffled negative control exceeded 6 hours")
    ...
    for name in [
        "stage2l_qwen_mask_shuffled_negative_control.json",
        "stage2l_qwen_mask_shuffled_negative_control.csv",
        "stage2l_llava_mask_shuffled_negative_control.json",
        "stage2l_llava_mask_shuffled_negative_control.csv",
    ]:
        try:
