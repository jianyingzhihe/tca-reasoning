...
def main() -> int:
    ...
    local_hidden_patch = (
        ...
        / "scripts"
        / "research"
        / "run_cross_model_hidden_patch_smoke.py"
    )
    remote_hidden_patch = f"{REMOTE_ROOT}/scripts/research/run_cross_model_hidden_patch_smoke.py"
    base._put_file(sftp, local_hidden_patch, remote_hidden_patch)
    sftp.chmod(remote_hidden_patch, 0o755)
    print("uploaded run_cross_model_hidden_patch_smoke.py")
    base._upload_assets(sftp)
    remote_script = f"{REMOTE_STAGE}/run_stage2g_hidden_patch.sh"
...
echo '--- Stage 2G hidden patch disk/gpu ---'
df -h /root/autodl-tmp
nvidia-smi --query-gpu=name,memory.total,memory.free --format=csv,noheader || true
.venv/bin/python -m py_compile scripts/research/run_cross_model_feature_intervention_smoke.py
.venv/bin/python -m py_compile scripts/research/run_cross_model_hidden_patch_smoke.py

echo '--- Qwen hidden-state clean patch upper-bound ---'
.venv/bin/python -u scripts/research/run_cross_model_hidden_patch_smoke.py \\
  --model-family qwen \\
  ...
  --work-dir "$STAGE/stage2g_qwen_hidden_patch_work" \\
  --samples okvqa_val_2847255,okvqa_val_4157235,okvqa_val_3658865 \\
  --prompts B_direct,D_visual_only \\
  --layer 26 \\
  --scales 0.5,1.0,1.5 \\
  --out-json "$STAGE/stage2g_qwen_hidden_patch_smoke.json" \\
  --out-csv "$STAGE/stage2g_qwen_hidden_patch_smoke.csv"

echo '--- LLaVA hidden-state clean patch upper-bound ---'
.venv/bin/python -u scripts/research/run_cross_model_hidden_patch_smoke.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --transcoder-ref KokosDev/llava15-7b-clt \\
  --annotation-roots "$ASSET_ROOT,/root/autodl-tmp/tca-reasoning/annotation/okvqa_evidence_labelme_round4_core24_easy,/root/autodl-tmp/tca-reasoning/annotation/okvqa_evidence_labelme_round4_mainline16" \\
  --work-dir "$STAGE/stage2g_llava_hidden_patch_work" \\
  --samples okvqa_val_2847255,okvqa_val_4157235,okvqa_val_3658865 \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  --scales 0.5,1.0,1.5 \\
  --out-json "$STAGE/stage2g_llava_hidden_patch_smoke.json" \\
  --out-csv "$STAGE/stage2g_llava_hidden_patch_smoke.csv"
"""
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 10800:
            ...
            raise TimeoutError("Stage 2G hidden patch exceeded 3 hours")
    ...
    for name in [
        "stage2g_qwen_hidden_patch_smoke.json",
        "stage2g_qwen_hidden_patch_smoke.csv",
        "stage2g_llava_hidden_patch_smoke.json",
        "stage2g_llava_hidden_patch_smoke.csv",
    ]:
        try:
