...
echo '--- Stage 2L wrong-target negative control disk/gpu ---'
...
echo '--- Stage 2L Qwen wrong-target negative control ---'
.venv/bin/python -u scripts/research/run_cross_model_wrong_target_negative_control_smoke.py \\
  ...
  --work-dir "$STAGE/stage2l_qwen_wrong_target_work" \\
  ...
  --random-controls 1 \\
  --out-json "$STAGE/stage2l_qwen_wrong_target_negative_control.json" \\
  --out-csv "$STAGE/stage2l_qwen_wrong_target_negative_control.csv"

echo '--- Stage 2L LLaVA wrong-target negative control ---'
.venv/bin/python -u scripts/research/run_cross_model_wrong_target_negative_control_smoke.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --annotation-roots "$ASSET_ROOT" \\
  --work-dir "$STAGE/stage2l_llava_wrong_target_work" \\
  --sample-manifest "$STAGE/stage2i_selected_12_manifest.csv" \\
  --samples manifest \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  ...
  --answer-adjacent-count 4 \\
  --random-controls 1 \\
  --out-json "$STAGE/stage2l_llava_wrong_target_negative_control.json" \\
  --out-csv "$STAGE/stage2l_llava_wrong_target_negative_control.csv"
"""
...
def _run_local_analysis() -> None:
    cmd = [
        sys.executable,
        str(ROOT / "vlm-circuit-tracing" / "circuit_tracer_vlm" / "scripts" / "research" / "analyze_stage2l_negative_controls.py"),
    ]
...
def main() -> int:
    ...
    remote_script = f"{REMOTE_STAGE}/run_stage2l_wrong_target_negative_control.sh"
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 21600:
            ...
            raise TimeoutError("Stage 2L wrong-target negative control exceeded 6 hours")
    ...
    for name in [
        "stage2l_qwen_wrong_target_negative_control.json",
        "stage2l_qwen_wrong_target_negative_control.csv",
        "stage2l_llava_wrong_target_negative_control.json",
        "stage2l_llava_wrong_target_negative_control.csv",
    ]:
        try:
