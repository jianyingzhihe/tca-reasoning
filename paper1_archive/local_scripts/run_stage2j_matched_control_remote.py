...
def _upload_assets(base, sftp, rows: list[dict[str, str]]) -> None:
    ...
    print(f"uploaded/verified Stage 2J assets for {len(rows)} selected samples")
...
echo '--- Stage 2J matched-control hidden-position patch disk/gpu ---'
...
echo '--- Stage 2J Qwen selected-12 matched-control restore/corrupt ---'
.venv/bin/python -u scripts/research/run_cross_model_hidden_position_patch_smoke.py \\
  ...
  --work-dir "$STAGE/stage2j_qwen_position_work" \\
  ...
  --random-controls 4 \\
  --out-json "$STAGE/stage2j_qwen_matched_control_patch.json" \\
  --out-csv "$STAGE/stage2j_qwen_matched_control_patch.csv"

echo '--- Stage 2J LLaVA selected-12 matched-control restore/corrupt ---'
.venv/bin/python -u scripts/research/run_cross_model_hidden_position_patch_smoke.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --annotation-roots "$ASSET_ROOT" \\
  --work-dir "$STAGE/stage2j_llava_position_work" \\
  --sample-manifest "$STAGE/stage2i_selected_12_manifest.csv" \\
  --samples manifest \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  ...
  --answer-adjacent-count 4 \\
  --random-controls 4 \\
  --out-json "$STAGE/stage2j_llava_matched_control_patch.json" \\
  --out-csv "$STAGE/stage2j_llava_matched_control_patch.csv"
"""
...
def _run_local_analysis() -> None:
    cmd = [
        ...
        str(LOCAL_STAGE),
        "--qwen-csv",
        "stage2j_qwen_matched_control_patch.csv",
        "--llava-csv",
        "stage2j_llava_matched_control_patch.csv",
        "--out-baseline-csv",
        str(LOCAL_STAGE / "stage2j_matched_control_baseline_gap.csv"),
        "--out-summary-csv",
        str(LOCAL_STAGE / "stage2j_matched_control_summary.csv"),
        "--out-specificity-csv",
        str(LOCAL_STAGE / "stage2j_matched_control_specificity.csv"),
        "--out-case-csv",
        str(LOCAL_STAGE / "stage2j_matched_control_case_table.csv"),
        "--out-decision-json",
        str(LOCAL_STAGE / "stage2j_matched_control_decision.json"),
    ]
...
def main() -> int:
    ...
    remote_script = f"{REMOTE_STAGE}/run_stage2j_matched_control_patch.sh"
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 21600:
            ...
            raise TimeoutError("Stage 2J matched-control patch exceeded 6 hours")
    ...
    for name in [
        "stage2j_qwen_matched_control_patch.json",
        "stage2j_qwen_matched_control_patch.csv",
        "stage2j_llava_matched_control_patch.json",
        "stage2j_llava_matched_control_patch.csv",
    ]:
        try:
