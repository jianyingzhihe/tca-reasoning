...
REMOTE_ASSETS = f"{REMOTE_STAGE}/stage2m_feature_assets"
...
def _model_samples(model: str) -> list[str]:
    selected: list[str] = []
    seen: set[str] = set()
    for row in _read_csv(PROMPT_ROWS):
...
def _upload_assets(base, sftp, rows: list[dict[str, str]]) -> None:
    ...
    print(f"uploaded/verified Stage 2M feature assets for {len(rows)} selected samples")
...
echo '--- Stage 2M feature bridge disk/gpu ---'
...
.venv/bin/python -m py_compile scripts/research/run_cross_model_feature_bridge_patch.py

echo '--- Stage 2M Qwen feature bridge on passing cases ---'
.venv/bin/python -u scripts/research/run_cross_model_feature_bridge_patch.py \\
  --model-family qwen \\
  --model-name "$QWEN_MODEL" \\
  --transcoder-ref KokosDev/qwen2p5vl-7b-clt \\
  --annotation-roots "$ASSET_ROOT" \\
  --work-dir "$STAGE/stage2m_qwen_feature_work" \\
  ...
  --top-k-features 4 \\
  --control-pool-size 512 \\
  --scale 1.0 \\
  --out-json "$STAGE/stage2m_qwen_feature_bridge.json" \\
  --out-csv "$STAGE/stage2m_qwen_feature_bridge.csv"

echo '--- Stage 2M LLaVA feature bridge on passing cases ---'
.venv/bin/python -u scripts/research/run_cross_model_feature_bridge_patch.py \\
  --model-family llava \\
  --model-name "$LLAVA_MODEL_DIR" \\
  --transcoder-ref KokosDev/llava15-7b-clt \\
  --annotation-roots "$ASSET_ROOT" \\
  --work-dir "$STAGE/stage2m_llava_feature_work" \\
  --sample-manifest "$MANIFEST" \\
  --samples "{llava_csv}" \\
  --prompts B_direct,D_visual_only \\
  --layer 15 \\
  ...
  --control-pool-size 512 \\
  --scale 1.0 \\
  --out-json "$STAGE/stage2m_llava_feature_bridge.json" \\
  --out-csv "$STAGE/stage2m_llava_feature_bridge.csv"
"""
...
def _run_local_analysis() -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(scripts / "analyze_stage2m_feature_bridge.py"),
            "--inputs",
            ",".join(
                [
                    str(LOCAL_STAGE / "stage2m_qwen_feature_bridge.csv"),
                    str(LOCAL_STAGE / "stage2m_llava_feature_bridge.csv"),
                ]
            ),
            "--out-summary",
            str(LOCAL_STAGE / "stage2m_feature_bridge_summary.csv"),
            "--out-specificity",
            str(LOCAL_STAGE / "stage2m_feature_bridge_specificity.csv"),
            "--out-decision",
            str(LOCAL_STAGE / "stage2m_feature_bridge_decision.json"),
        ],
        cwd=str(ROOT),
        check=True,
    )
...
def main() -> int:
    ...
    if not qwen_samples or not llava_samples:
        raise RuntimeError("Feature candidate list must contain both qwen and llava samples.")
    ...
    for name in [
        ...
        "run_cross_model_feature_bridge_patch.py",
    ]:
        _upload_research_script(base, sftp, name)
    _upload_assets(base, sftp, rows)
    remote_script = f"{REMOTE_STAGE}/run_stage2m_feature_bridge.sh"
    ...
    while not stdout.channel.exit_status_ready():
        ...
        if time.time() - start > 21600:
            ...
            raise TimeoutError("Stage 2M feature bridge exceeded 6 hours")
    ...
    for name in [
        "stage2m_qwen_feature_bridge.json",
        "stage2m_qwen_feature_bridge.csv",
        "stage2m_llava_feature_bridge.json",
        "stage2m_llava_feature_bridge.csv",
    ]:
        try:
