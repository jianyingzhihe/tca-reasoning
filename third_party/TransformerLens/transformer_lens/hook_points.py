...
class HookedRootModule(nn.Module):
    ...
    def run_with_hooks(
        ...
        **model_kwargs,
        ...
        If a `batch` kwarg is provided, we bypass text/image preprocessing and
        call forward_from_batch so that media_locations is honored.
        """
        ...
        batch = model_kwargs.pop("batch", None)
        stop_at_layer = model_kwargs.pop("stop_at_layer", None)

        with self.hooks(fwd_hooks, bwd_hooks, reset_hooks_end, clear_contexts) as hooked_model:
            if batch is not None:
                # Use the exact processor outputs (includes media mask)
                return hooked_model.forward_from_batch(batch, stop_at_layer=stop_at_layer)
            else:
                # Legacy path (text-only or custom arg flow)
                return hooked_model.forward(*model_args, stop_at_layer=stop_at_layer, **model_kwargs)
