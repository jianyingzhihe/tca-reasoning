...
class HookedVLTransformer(HookedRootModule):
    ...
    def input_to_embed(
        ...
        if isinstance(input, str) or isinstance(input, list):
            ...
            if self._is_image_list_non_empty(images):
                # some processors require text of the same batch size; use empty strings
                b = tokens.shape[0] if tokens.ndim == 2 else 1
                dummy_text = ["<start_of_image>The object in the picture is"] * b
                ...
                pixel_values = vis_inputs.get("pixel_values", None)

                if self.vl_strategy == "cross":
                    aspect_ratio_ids = vis_inputs["aspect_ratio_ids"]
                    aspect_ratio_mask = vis_inputs["aspect_ratio_mask"]
                    cross_attention_mask = vis_inputs["cross_attention_mask"]
                    image_grid_thw = None
                else:
                    aspect_ratio_ids = aspect_ratio_mask = cross_attention_mask = None
                    image_grid_thw = vis_inputs.get("image_grid_thw", None)
    ...
    def forward(
        ...
        with utils.LocallyOverridenDefaults(
            ...
            if images_exist and "gemma" in self.cfg.model_name:
    ...
    def to_tokens(
        ...
        move_to_device: bool = True,
        truncate: bool = True,
        return_also_image_values: bool = True,
    ) -> Int[torch.Tensor, "batch pos"]:
        ...
        with utils.LocallyOverridenDefaults(
            ...
            if images is None:
                ...
                single_image_model = "llava" in self.cfg.model_name.lower()
            ...
            if return_also_image_values:
                return (tokens, pixel_values, aspect_ratio_ids, aspect_ratio_mask, cross_attn_mask, image_grid_thw)
                
            else:
                raise ValueError("FWEFEWFW")
                return tokens
